...
const MovieManager = () => {
  ...
  const handleOpen = (movie = null) => {
    ...
    setMovieStatus(movie ? movie.status : true);
    setMovieData(
      movie
  ...
  //   const updatedMovie = { ...movie, status: !movie.status };
  //   try {
  ...
  //   setMovieData({ ...movieData, status: e.target.checked });
  // };

  const handleSwitchChange = async (movie) => {
    const updatedMovie = { ...movie, status: !movie.status };
    try {
      await updateMovie(movie.id, updatedMovie);
      fetchMovies();
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <Box width="48%" sx={{ pl: 2 }}>
              ...
              <FormControlLabel
                ...
                label={movieData.status ? "Active" : "Inactive"}
              />
            </Box>
          </Box>
